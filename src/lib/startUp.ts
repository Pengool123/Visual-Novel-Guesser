...
import {currVN, reset} from './gameMech';
...
function itemToVN(item:any){
    //jank way of getting if it has ero or not
        ...
        item.titles.map((titles:any) => {
            if(!aliasList.includes(titles.title)){
        ...
        if(relVNList.length !== 0){
            ...
            while(j < newRelVNList.length){
                ...
                if(tempVN !== undefined){
                    ...
                    const seenIds = new Set(newRelVNList.map(r => r.id));
                    ...
                    if (newRelVNList[j].connection === Relation.Prequel) {
                        tempVN.relatedTitles.forEach((relVn: RelatedVN) => {
                            ...
                            if (relVn.connection === Relation.Prequel && !seenIds.has(relVn.id) && temp !== undefined) {
                                // prevent duplicates added in this same loop
                                seenIds.add(relVn.id);
                                newRelVNList.push(new RelatedVN({
                                    title: temp.title,
                                    id: temp.id,
                    ...
                    if (newRelVNList[j].connection === Relation.Sequel) {
                        tempVN.relatedTitles.forEach((relVn: RelatedVN) => {
                            const temp: any = VNMap.get(relVn.id);
                            if (relVn.connection === Relation.Sequel && !seenIds.has(relVn.id) && temp !== undefined) {
                                seenIds.add(relVn.id);
                                newRelVNList.push(new RelatedVN({
                                    title: temp.title,
                                    id: temp.id,
